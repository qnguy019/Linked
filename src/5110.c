...
//Function that I made myself
void lcd_clear_space(unsigned char line_start, unsigned char line_end){
...
//Function I added myself
void printPictureAtPos(const unsigned char *data, unsigned char x, unsigned char y, unsigned char width, unsigned char height)
{
	int pixel_cols = width * (height / CHAR_HEIGHT); //<6> means 6 lines on LCD.
	lcd_goto_xy(x, y);
